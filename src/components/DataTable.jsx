...
const DataTable = ({
  ...
  const pages = [{key:10,label:10},{key:25,label:25},{key:50,label:50},{key:100,label:100}];
  return (
    <div className="px-2 py-4 grid grid-cols-5 gap-4">
          ...
          {filters.length > 0 && (
            <div className="h-fit">
              ...
              {/* <div className="flex gap-2 border-t px-2 py-2 bg-white rounded-md border border-gray-300  shadow-lg"> */}
                ...
                <Button type="button" variant="secondary" onClick = {()=>{setFilterVals(getInitialFilters())}}>
                  Clear
                </Button>
              </div>
            </div>
      ...
      <div className={`${filters.length > 0 && showFilter ? "col-span-4" : "col-span-5"}`}>
        <div className="border border-gray-300 bg-white rounded-xl h-auto px-3 py-3 shadow-sm flex flex-col gap-4 md:flex-row md:items-center md:justify-between sm:h-12">
          ...
          <PanelLeft className="cursor-pointer text-gray-500" onClick={()=>{setShowFilter(prev=>!prev)}}/>
            {is_search && (
              <TextInput
                name="search"
                type="text"
